...
public class DAISMakeOrderNRServlet extends HttpServlet {
	
	private static final long serialVersionUID = 5204357422698934527L;
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			String code = port.addGuestApplication(order);
			
			if (code != null) {
				request.getSession().setAttribute("code", code);
				RequestDispatcher disp = getServletContext().getRequestDispatcher("/code.jsp");
				if (disp != null) {
